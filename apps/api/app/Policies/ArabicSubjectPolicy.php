<?php

declare(strict_types=1);

namespace App\Policies;

use App\Domain\Arabic\Models\ArabicSubject;
use App\Models\User;

/**
 * Table de référence globale (comme Subject) : provisionnée uniquement par
 * le Super Admin SaaS via le bypass Gate::before (AppServiceProvider) —
 * tout le monde d'autre est refusé explicitement.
 */
class ArabicSubjectPolicy
{
    public function viewAny(User $user): bool
    {
        return false;
    }

    public function view(User $user, ArabicSubject $arabicSubject): bool
    {
        return false;
    }

    public function create(User $user): bool
    {
        return false;
    }

    public function update(User $user, ArabicSubject $arabicSubject): bool
    {
        return false;
    }

    public function delete(User $user, ArabicSubject $arabicSubject): bool
    {
        return false;
    }
}
