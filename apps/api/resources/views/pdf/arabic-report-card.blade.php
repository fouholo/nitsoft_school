...
<head>
    ...
    <style>
        ...
        table.grades { width: 100%; border-collapse: collapse; margin-bottom: 20px; direction: rtl; font-family: 'NotoNaskhArabic', "DejaVu Sans", sans-serif; }
        table.grades th, table.grades td { border: 1px solid #cbd5e1; padding: 6px 8px; text-align: right; }
...
<body>
    <div class="header">
        ...
        <p>Bulletin (filière arabe) — {{ $reportCard->arabicTerm->label }} ({{ $reportCard->arabicTerm->schoolYear?->label }})</p>
    </div>

    <table class="meta">
        <tr>
            <td class="label">Élève</td>
            <td>{{ $reportCard->enrollment->student->last_name }} {{ $reportCard->enrollment->student->first_name }}</td>
            <td class="label">Matricule</td>
            <td>{{ $reportCard->enrollment->student->student_number }}</td>
        </tr>
        <tr>
    ...
    <table class="grades">
        <thead>
            <tr>
                <th class="col-disciplines">المادة</th>
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="6">Aucune note disponible pour cette période.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
