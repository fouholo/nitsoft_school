<div class="mt-4 rounded-2xl border border-stone-200 bg-white p-5">
    <p class="text-sm font-semibold uppercase tracking-wide text-stone-500">Alertes</p>

    @if (count($items) === 0)
        <p class="mt-3 text-sm text-stone-500">Aucun point d'attention pour l'instant.</p>
    @else
                ...
                <li class="flex items-center justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                    <span>
                        ...
                        @if ($showEstablishmentTag)
                            <span class="ml-1 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-700">{{ $item['establishmentName'] }}</span>
                        @endif
                    </span>
