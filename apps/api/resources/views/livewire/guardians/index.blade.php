<div>
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900">{{ __('Tuteurs') }}</h1>
    ...
    <div class="mt-4">
        <input
            ...
            placeholder="{{ __('Rechercher un tuteur...') }}"
    ...
    @if ($unlinkedGuardianNotice)
        <div class="mt-4 flex items-start justify-between gap-4 rounded-lg border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-900">
            <p>{!! __(':name a été créé. Il n\'apparaîtra dans cette liste qu\'une fois lié à un élève — depuis la fiche de l\'élève, utilisez « Ajouter un tuteur » pour le rechercher et le rattacher.', ['name' => '<strong>'.e($unlinkedGuardianNotice).'</strong>']) !!}</p>
    ...
    @if ($generatedPassword)
        <div class="mt-4 rounded-lg border border-amber-200 bg-amber-100 px-4 py-3 text-sm text-amber-900" role="alert" aria-live="assertive">
            <p class="font-semibold">{{ __('Compte portail créé pour :email', ['email' => $generatedPasswordFor]) }}</p>
            <p class="mt-1">{{ __('Mot de passe par défaut à communiquer au tuteur — invitez-le à le changer dès sa première connexion (menu de son profil) :') }}</p>
    ...
    @if ($showForm)
        <form wire:submit="save" class="mt-4 grid grid-cols-1 gap-4 rounded-lg border border-stone-200 bg-white p-4 sm:grid-cols-3">
            ...
            <label class="flex items-center gap-2 text-sm text-stone-600 sm:col-span-3">
                ...
                {{ __("Créer un compte d'accès au portail parents (nécessite une adresse e-mail)") }}
            </label>
            ...
            <table class="min-w-full divide-y divide-stone-200 text-sm">
                <thead class="bg-stone-50">
                    <tr>
                        ...
                        <th class="whitespace-nowrap px-4 py-2 text-start font-medium text-stone-500">{{ __('E-mail') }}</th>
                        <th class="whitespace-nowrap px-4 py-2 text-start font-medium text-stone-500">{{ __('Portail') }}</th>
                ...
                <tbody class="divide-y divide-stone-100">
                    @forelse ($guardians as $guardian)
                        <tr wire:key="guardian-{{ $guardian->id }}">
                            ...
                            <td class="whitespace-nowrap px-4 py-2 text-end">
                                @can('update', $guardian)
                                    <button wire:click="edit({{ $guardian->id }})" class="inline-flex min-h-11 items-center text-stone-500 hover:text-stone-900">{{ __('Modifier') }}</button>
                                @endcan
                                @can('delete', $guardian)
                                    <button
                                        wire:click="delete({{ $guardian->id }})"
                                        wire:confirm="{{ __('Supprimer ce tuteur ?') }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-stone-500">{{ __('Aucun tuteur.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
