<div>
    <h1 class="text-2xl font-semibold text-slate-900">Envoyer un SMS</h1>

    @if ($confirmation)
        <div class="mt-4 rounded-md border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            {{ $confirmation }}
        </div>
    @endif

    <form wire:submit="send" class="mt-6 max-w-lg space-y-4 rounded-md border border-slate-200 bg-white p-4">
        <div>
            <label class="block text-sm font-medium text-slate-700">Élève</label>
            <select wire:model.live="student_id" class="mt-1 block w-full rounded-md border-slate-300 text-sm">
        ...
        <div>
            ...
            <textarea wire:model="body" rows="4" class="mt-1 block w-full rounded-md border-slate-300 text-sm" maxlength="480"></textarea>
