<div>
    <a href="{{ route('foundations.index') }}" wire:navigate class="text-sm text-stone-500 hover:text-stone-900">&larr; {{ __('Retour aux groupes scolaires') }}</a>
        ...
        <section>
            ...
            <div class="mt-2 overflow-hidden rounded-lg border border-stone-200 bg-white">
                <table class="min-w-full divide-y divide-stone-200 text-sm">
                    <tbody class="divide-y divide-stone-100">
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-6 text-center text-stone-500">{{ __('Aucun établissement rattaché.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <form wire:submit="linkEstablishment" class="mt-3 flex gap-2">
                <select wire:model="establishment_to_link" class="block w-full rounded-lg border-stone-300 text-sm">
                    <option value="">{{ __('Rattacher un établissement existant…') }}</option>
            ...
            <div class="mt-2 overflow-hidden rounded-lg border border-stone-200 bg-white">
                <table class="min-w-full divide-y divide-stone-200 text-sm">
                    <tbody class="divide-y divide-stone-100">
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-6 text-center text-stone-500">{{ __('Aucun fondateur rattaché.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
