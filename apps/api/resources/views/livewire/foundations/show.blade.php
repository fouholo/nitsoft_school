<div>
    <a href="{{ route('foundations.index') }}" wire:navigate class="text-sm text-slate-500 hover:text-slate-900">&larr; Retour aux groupes scolaires</a>

    <h1 class="mt-2 text-2xl font-semibold text-slate-900">{{ $foundation->name }}</h1>
    ...
    <div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
        <section>
            ...
            <div class="mt-2 overflow-hidden rounded-md border border-slate-200 bg-white">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-6 text-center text-slate-500">Aucun établissement rattaché.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <form wire:submit="linkEstablishment" class="mt-3 flex gap-2">
                <select wire:model="establishment_to_link" class="block w-full rounded-md border-slate-300 text-sm">
            ...
            @if ($availableEstablishments->isEmpty())
                <p class="mt-2 text-sm text-slate-400">Aucun établissement indépendant disponible à rattacher.</p>
            @endif
        </section>
        ...
        <section>
            ...
            <div class="mt-2 overflow-hidden rounded-md border border-slate-200 bg-white">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($founders as $founder)
                            <tr wire:key="founder-{{ $founder->id }}">
                                <td class="px-4 py-2 text-slate-900">
                                    {{ $founder->user->name }}
                                    <span class="block text-xs text-slate-500">{{ $founder->user->email }}</span>
                                </td>
                                <td class="px-4 py-2 text-right">
                                    <button
                        ...
                        @empty
                            <tr>
                                <td class="px-4 py-6 text-center text-slate-500">Aucun fondateur rattaché.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <form wire:submit="addFounder" class="mt-3 space-y-2 rounded-md border border-slate-200 bg-white p-3">
                <div>
                    <label class="block text-xs font-medium text-slate-700">Nom</label>
                    <input type="text" wire:model="founder_name" class="mt-1 block w-full rounded-md border-slate-300 text-sm">
                ...
                <div>
                    ...
                    <input type="email" wire:model="founder_email" class="mt-1 block w-full rounded-md border-slate-300 text-sm">
