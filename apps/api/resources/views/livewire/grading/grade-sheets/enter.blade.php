<div>
    ...
    <div class="mt-2">
        ...
        <p class="text-sm text-stone-500">
            {{ $gradeSheet->classroom?->name }} — {{ $gradeSheet->subject?->name }} — {{ __('Barème :max', ['max' => $gradeSheet->max_score]) }} — {{ $gradeSheet->term?->label }}
            @if ($totalCount > 0)
                ...
                <table class="min-w-full divide-y divide-stone-200 text-sm">
                    ...
                    <tbody class="divide-y divide-stone-100">
                        @forelse ($students as $student)
                            <tr wire:key="grade-row-{{ $student->id }}" class="odd:bg-white even:bg-stone-50/60">
