<div>
    ...
    <h1 class="mt-2 text-2xl font-semibold text-stone-900">Facturation — {{ $student->last_name }} {{ $student->first_name }}</h1>

    @if (! $enrollment)
        <p class="mt-6 text-sm text-stone-500">Aucune inscription active pour cette année scolaire.</p>
    @else
        <div class="mt-4 grid grid-cols-1 gap-4 rounded-lg border border-stone-200 bg-white p-4 sm:grid-cols-3">
            <div>
                <p class="text-xs font-medium uppercase tracking-wide text-stone-400">Total dû</p>
                <p class="text-sm text-stone-900">{{ money($totalDue) }}</p>
            </div>
            <div>
                <p class="text-xs font-medium uppercase tracking-wide text-stone-400">Total versé</p>
                <p class="text-sm text-stone-900">{{ money($totalPaid) }}</p>
            </div>
            <div>
                <p class="text-xs font-medium uppercase tracking-wide text-stone-400">Solde</p>
                @if ($balance > 0)
        ...
        <div class="mt-2 overflow-hidden rounded-lg border border-stone-200 bg-white">
            <table class="min-w-full divide-y divide-stone-200 text-sm">
                <thead class="bg-stone-50">
                    <tr>
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Poste</th>
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Échéance</th>
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Montant</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stone-100">
                    <tr>
                        <td class="px-4 py-2 text-stone-900">Frais d'inscription</td>
                    ...
                    @foreach ($tuitionInstallments as $installment)
                        <tr>
                            <td class="px-4 py-2 text-stone-900">Tranche {{ $installment['position'] }}</td>
            ...
            <table class="min-w-full divide-y divide-stone-200 text-sm">
                <thead class="bg-stone-50">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Montant</th>
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Moyen</th>
                        <th class="px-4 py-2 text-left font-medium text-stone-500">Reçu</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stone-100">
                    @forelse ($payments as $payment)
                        <tr wire:key="guardian-payment-{{ $payment->id }}">
                            ...
                            <td class="px-4 py-2 text-stone-600">{{ $payment->method }}</td>
                            <td class="px-4 py-2 text-stone-600">{{ $payment->receiptNumber() }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-6 text-center text-stone-500">Aucun paiement enregistré.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
