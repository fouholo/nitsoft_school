<div>
    <h1 class="text-2xl font-semibold text-stone-900">{{ __('Lier un enfant') }}</h1>
    <p class="mt-1 text-sm text-stone-500">
        {{ __("Saisissez le code (uid) de l'élève, fourni par l'établissement, pour demander à être lié à son dossier.") }}
    </p>
    ...
    <div class="mt-6 max-w-md rounded-lg border border-stone-200 bg-white p-4">
        <form wire:submit="search" class="flex items-end gap-2">
            <div class="flex-1">
                <label for="uid" class="block text-sm font-medium text-stone-700">{{ __("Code de l'élève") }}</label>
                <input
                    type="text"
                    id="uid"
                    wire:model="uid"
        ...
        @if ($foundStudent)
            ...
            <form wire:submit="requestLink" class="mt-4 space-y-3">
                <div>
                    <label for="relationship" class="block text-sm font-medium text-stone-700">{{ __('Votre rôle auprès de cet élève') }}</label>
