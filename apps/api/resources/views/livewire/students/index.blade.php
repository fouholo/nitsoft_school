<div>
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900">{{ __('Élèves') }}</h1>
    ...
    @if (! empty($generatedAccounts))
        <div class="mt-4 rounded-lg border border-amber-200 bg-amber-100 px-4 py-3 text-sm text-amber-900" role="alert" aria-live="assertive">
            <p class="font-semibold">{{ count($generatedAccounts) > 1 ? __('Comptes portail créés') : __('Compte portail créé') }}</p>
            <p class="mt-1">{{ __('Mot de passe par défaut à communiquer aux parents — invitez-les à le changer dès leur première connexion (menu de leur profil) :') }}</p>
    ...
    @if ($showForm)
        <div class="mt-4 rounded-lg border border-stone-200 bg-white p-4" x-data x-init="$el.scrollIntoView({ behavior: 'smooth', block: 'start' })">
            @php
                ...
                $stepLabels = [__('Identité'), __('Acte de naissance et nationalité'), __('Contacts familiaux')];
            @endphp

            <h2 class="text-lg font-semibold text-stone-900">
                {{ $editingId ? __('Modification') : __('Nouvel élève') }}{{ $formSubtitle !== '' ? ' — '.$formSubtitle : '' }}
            </h2>
            ...
            <p class="mt-2 text-xs font-medium uppercase tracking-wide text-stone-500">
                {{ __('Étape :current sur :total — :label', ['current' => $currentStep, 'total' => count($stepLabels), 'label' => $stepLabels[$currentStep - 1]]) }}
            </p>

            <form wire:submit="save" class="mt-4">
                @if ($currentStep === 1)
                    <p class="mb-3 text-xs text-stone-500">{!! __("Les champs marqués d'un :star sont obligatoires.", ['star' => '<span class="text-red-600">*</span>']) !!}</p>

                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                        <div>
                            <label for="last_name" class="block text-sm font-medium text-stone-700">{{ __('Nom') }} <span class="text-red-600">*</span></label>
                        ...
                        <div>
                            <label for="first_name" class="block text-sm font-medium text-stone-700">{{ __('Prénom') }} <span class="text-red-600">*</span></label>
                        ...
                        <div>
                            <label for="student_number" class="block text-sm font-medium text-stone-700">{{ __('Matricule') }}</label>
                        ...
                        <div>
                            <label for="gender" class="block text-sm font-medium text-stone-700">{{ __('Genre') }}</label>
                        ...
                        <div>
                            <label for="birth_date" class="block text-sm font-medium text-stone-700">{{ __('Date de naissance') }}</label>
                        ...
                        <div>
                            <label for="birth_place" class="block text-sm font-medium text-stone-700">{{ __('Lieu de naissance') }}</label>
                        ...
                        <div class="sm:col-span-2">
                            <label for="photo" class="block text-sm font-medium text-stone-700">{{ __('Photo') }}</label>
                            <input id="photo" type="file" wire:model="photo" accept=".jpg,.jpeg,.png,.webp,.gif" class="mt-1 block w-full text-sm">
                            <p class="mt-1 text-xs text-stone-500">{{ __('JPG, PNG, WebP ou GIF, 100 Ko max.') }}</p>
                            @error('photo') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror

                            @if ($photo)
                                <img src="{{ $photo->temporaryUrl() }}" alt="{{ __('Aperçu') }}" class="mt-2 h-16 w-16 rounded-lg object-cover">
                            @elseif ($existingPhotoPath)
                                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($existingPhotoPath) }}" alt="{{ __('Photo actuelle') }}" class="mt-2 h-16 w-16 rounded-lg object-cover">
                            @endif
                        </div>
                    </div>
                @endif
                ...
                @if ($currentStep === 2)
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-4">
                        <div>
                            <label for="nationalite_code" class="block text-sm font-medium text-stone-700">{{ __('Nationalité') }}</label>
                        ...
                        <div>
                            <label for="birth_certificate_number" class="block text-sm font-medium text-stone-700">{{ __('N° acte de naissance') }}</label>
                        ...
                        <div>
                            <label for="birth_certificate_date" class="block text-sm font-medium text-stone-700">{{ __("Date de l'acte de naissance") }}</label>
                        ...
                        <div>
                            <label for="birth_certificate_place" class="block text-sm font-medium text-stone-700">{{ __("Lieu de l'acte de naissance") }}</label>
                        ...
                        <div>
                            <label for="residence" class="block text-sm font-medium text-stone-700">{{ __('Résidence') }}</label>
                ...
                @if ($currentStep === 3)
                    <p class="mb-3 text-xs text-stone-500">{{ __("Cochez « Créer et lier un compte » pour donner à un parent l'accès au portail parents — un mot de passe sera généré et affiché une seule fois.") }}</p>

                    <div class="space-y-4">
                        @foreach ([
                            ['key' => 'father', 'label' => __('Père'), 'nameField' => 'father_name', 'phoneField' => 'father_phone', 'emailField' => 'fatherEmail', 'createField' => 'createAccountForFather'],
                            ['key' => 'mother', 'label' => __('Mère'), 'nameField' => 'mother_name', 'phoneField' => 'mother_phone', 'emailField' => 'motherEmail', 'createField' => 'createAccountForMother'],
                            ['key' => 'tutor', 'label' => __('Tuteur'), 'nameField' => 'tutor_name', 'phoneField' => 'tutor_phone', 'emailField' => 'tutorEmail', 'createField' => 'createAccountForTutor'],
                        ] as $row)
                            <div wire:key="family-row-{{ $row['key'] }}" class="rounded-lg border border-stone-200 p-4">
                                ...
                                <div class="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <div>
                                        <label for="{{ $row['nameField'] }}" class="block text-sm font-medium text-stone-700">{{ __('Nom') }}</label>
                                    ...
                                    <div>
                                        <label for="{{ $row['phoneField'] }}" class="block text-sm font-medium text-stone-700">{{ __('Téléphone') }}</label>
                                ...
                                @if (${$row['createField']})
                                    <div class="mt-3">
                                        <label for="{{ $row['emailField'] }}" class="block text-sm font-medium text-stone-700">
                                            {{ __('E-mail — :label', ['label' => $row['label']]) }} <span class="text-red-600">*</span>
                                        </label>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-stone-200 text-sm">
                <thead class="bg-stone-50">
                    <tr>
                        <th class="whitespace-nowrap px-4 py-2 text-start font-medium text-stone-500">{{ __('Matricule') }}</th>
                        <th class="whitespace-nowrap px-4 py-2 text-start font-medium text-stone-500">{{ __('Nom') }}</th>
                        <th class="whitespace-nowrap px-4 py-2 text-start font-medium text-stone-500">{{ __('Date de naissance') }}</th>
                ...
                <tbody class="divide-y divide-stone-100">
                    @forelse ($students as $student)
                        <tr wire:key="student-{{ $student->id }}" class="{{ $editingId === $student->id ? 'bg-orange-50' : '' }}">
                            ...
                            <td class="whitespace-nowrap px-4 py-2 text-end">
                                @can('update', $student)
                                    <button wire:click="edit({{ $student->id }})" class="text-stone-500 hover:text-stone-900">{{ __('Modifier') }}</button>
                                @endcan
                                @can('delete', $student)
                                    <button
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-6 text-center text-stone-500">{{ __('Aucun élève.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
