<div class="relative" x-data="{ open: false }">
    ...
    <div
        ...
        x-cloak
        class="absolute right-0 z-10 mt-2 w-56 rounded-xl border border-stone-200 bg-white py-1 shadow-lg"
    >
        @forelse ($establishments as $establishment)
            <button
                ...
                @click="open = false"
                class="block w-full px-4 py-2 text-left text-sm {{ $establishment->id === $currentEstablishmentId ? 'font-semibold text-orange-800 bg-orange-50' : 'text-stone-700 hover:bg-stone-50' }}"
            >
                {{ $establishment->name }}
            </button>
        @empty
            <p class="px-4 py-2 text-sm text-stone-500">Aucun établissement associé.</p>
        @endforelse
    </div>
</div>
