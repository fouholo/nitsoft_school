<div>
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900">{{ __('Périodes arabes') }}</h1>
    ...
    @if ($showForm)
        <form wire:submit="save" class="mt-4 grid grid-cols-1 gap-4 rounded-lg border border-stone-200 bg-white p-4 sm:grid-cols-5">
            <div>
                ...
                <input type="text" wire:model="label" placeholder="{{ __('Période 1') }}" class="mt-1 block w-full rounded-lg border-stone-300 text-sm">
            ...
            <div>
                <label class="block text-sm font-medium text-stone-700">{{ __('Début (facultatif)') }}</label>
            ...
            <div>
                <label class="block text-sm font-medium text-stone-700">{{ __('Fin (facultatif)') }}</label>
            ...
            <p class="text-xs text-stone-500 sm:col-span-5">{{ __('Laissez les dates vides pour une période sans dates fixes (utilisée comme simple numéro de composition).') }}</p>
                ...
                @forelse ($arabicTerms as $arabicTerm)
                    <tr wire:key="arabic-term-{{ $arabicTerm->id }}">
                        ...
                        <td class="px-4 py-2 text-stone-600">
                            ...
                            @else
                                {{ __('Composition n°:number', ['number' => $arabicTerm->sequence]) }}
                            @endif
                        </td>
                        <td class="px-4 py-2 text-stone-600">{{ $arabicTerm->schoolYear?->label }}</td>
                        <td class="px-4 py-2 text-end">
                            @can('update', $arabicTerm)
                                <button wire:click="edit({{ $arabicTerm->id }})" class="text-stone-500 hover:text-stone-900">{{ __('Modifier') }}</button>
                            @endcan
