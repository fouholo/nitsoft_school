<div>
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-semibold text-stone-900">Tarifs</h1>
    ...
    @if (! $school_year_id)
        <p class="mt-6 text-sm text-stone-500">Sélectionnez une année scolaire pour gérer ses tarifs.</p>
    @else
        {{-- Tranches --}}
        <div class="mt-8">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-stone-900">Tranches</h2>

                <div class="flex items-center gap-3">
                    @if ($installmentJustSaved)
                        <span class="rounded-lg bg-emerald-50 px-3 py-1.5 text-sm text-emerald-700">Tranche enregistrée.</span>
                    @endif
            ...
            @if ($showInstallmentForm)
                <form wire:submit="saveInstallment" class="mt-4 grid grid-cols-1 gap-4 rounded-lg border border-stone-200 bg-white p-4 sm:grid-cols-3">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" wire:model="label" placeholder="Octobre" class="mt-1 block w-full rounded-lg border-stone-300 text-sm">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-stone-700">Échéance</label>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-stone-200 text-sm">
                        <thead class="bg-stone-50">
                            <tr>
                                <th class="whitespace-nowrap px-4 py-2 text-left font-medium text-stone-500">Ordre</th>
                        ...
                        <tbody class="divide-y divide-stone-100">
                                ...
                                <tr wire:key="installment-{{ $installment->id }}">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-right">
                                        @can('update', $installment)
                                            <button wire:click="editInstallment({{ $installment->id }})" class="text-stone-500 hover:text-stone-900">Modifier</button>
                                        @endcan
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-stone-500">Aucune tranche pour cette année scolaire.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        {{-- Tarifs par niveau --}}
        <div class="mt-8">
            <h2 class="text-lg font-semibold text-stone-900">Tarifs par niveau</h2>

            @if ($installments->isEmpty())
                <p class="mt-2 text-sm text-stone-500">Aucune tranche n'est encore définie pour cette année scolaire — vous pouvez tout de même configurer les frais d'inscription par niveau.</p>
            @endif
            ...
            <div class="mt-4 overflow-hidden rounded-lg border border-stone-200 bg-white">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-stone-200 text-sm">
                        <thead class="bg-stone-50">
                            <tr>
                                <th class="whitespace-nowrap px-4 py-2 text-left font-medium text-stone-500">Niveau</th>
                                <th class="whitespace-nowrap px-4 py-2 text-left font-medium text-stone-500">Frais d'inscription</th>
                                <th class="whitespace-nowrap px-4 py-2 text-left font-medium text-stone-500">Total scolarité configuré</th>
                                <th class="whitespace-nowrap px-4 py-2"></th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-stone-100">
                                ...
                                <tr wire:key="level-{{ $level->id }}">
                                    ...
                                    <td class="whitespace-nowrap px-4 py-2 text-right">
                                        @if ($levelFeeJustSaved && $lastSavedLevelId === $level->id)
                                            <span class="mr-3 rounded-lg bg-emerald-50 px-2 py-1 text-xs text-emerald-700">Tarifs enregistrés.</span>
                                        @endif
                                        @can('create', \App\Domain\Billing\Models\LevelFee::class)
                                            <button wire:click="configureLevel({{ $level->id }})" class="text-stone-500 hover:text-stone-900">Configurer</button>
                                        @endcan
                                    </td>
                                </tr>
                                            ...
                                            >
                                                @if ($otherConfiguredLevelIds->isNotEmpty())
                                                    <div class="col-span-2 sm:col-span-4">
                                                        <label class="block text-sm font-medium text-stone-700">Dupliquer les montants depuis un autre niveau</label>
                                                        ...
                                                        <p class="mt-1 text-xs text-stone-500">Les montants seront pré-remplis ; vous pourrez les ajuster avant d'enregistrer.</p>
                                                    </div>
                                                @endif

                                                <div>
                                                    <label class="block text-sm font-medium text-stone-700">Frais d'inscription {{ $configuringLevelIsSecondaire ? '(non affecté)' : '' }} — F CFA</label>
                                                ...
                                                @if ($configuringLevelIsSecondaire)
                                                    <div>
                                                        <label class="block text-sm font-medium text-stone-700">Frais d'inscription (affecté) — F CFA</label>
                                                    ...
                                                    <p class="col-span-2 -mt-2 text-xs text-stone-500 sm:col-span-4">Le montant « affecté » s'applique une fois l'élève affecté à une série ; « non affecté » s'applique avant cette affectation.</p>
                                                @endif
                                                ...
                                                <div class="col-span-2 flex items-center justify-between rounded-lg bg-white px-3 py-2 text-sm sm:col-span-4" x-data>
                                                    <span class="font-medium text-stone-700">Total scolarité (aperçu)</span>
