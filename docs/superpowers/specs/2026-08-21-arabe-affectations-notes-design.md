...
5. **Accès à la saisie de notes** : même largeur d'accès que le français — fondateur/directeur/gestionnaire/educateur ont un accès large (toutes les grilles arabes de l'établissement), en plus de l'enseignant affecté qui ne voit que ses groupes. Différent du choix fait pour les coefficients (sous-chantier 1, educateur exclu) car la saisie de notes est une tâche pédagogique quotidienne, pas une action de configuration.
...
- **`ArabicGradeSheetPolicy`** : `viewAny`/`view` = membre établissement `is_arabe`. `create` = `in_array($user->currentRole(), ['fondateur','directeur','gestionnaire','educateur'], true)` **ou** l'utilisateur a une `ArabicTeacherAssignment` correspondant au `(arabic_level_id, arabic_serie_id, arabic_subject_id)` visé. `update` (saisie des notes via `Enter`) : même règle, vérifiée sur la grille existante.
